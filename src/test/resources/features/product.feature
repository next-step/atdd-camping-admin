Feature: 상품을 관리자가 관리한다
  ...
  Scenario: 관리자가 새 상품을 생성하면 성공한다
    When 관리자가 "테스트 캠핑의자" 상품을 50000원에 생성한다
    Then 상품 이름이 "테스트 캠핑의자"이고 가격이 50000원인 상품이 생성된다

  Scenario: 관리자가 필수 정보 없이 상품을 생성하면 실패한다
    When 관리자가 상품명 없이 상품을 생성한다
    Then 잘못된 요청으로 인해 실패한다

  Scenario: 관리자가 잘못된 상품 타입으로 상품을 생성하면 실패한다
    When 관리자가 "테스트 캠핑텐트" 상품을 잘못된 타입으로 생성한다
    Then 잘못된 요청으로 인해 실패한다

  Scenario: 관리자가 음수 가격으로 상품을 생성하면 실패한다
    When 관리자가 "테스트 상품" 상품을 음수 가격으로 생성한다
    Then 잘못된 요청으로 인해 실패한다

  Scenario: 관리자가 음수 재고로 상품을 생성하면 실패한다
    When 관리자가 "테스트 상품" 상품을 음수 재고로 생성한다
    Then 잘못된 요청으로 인해 실패한다

  Scenario: 권한 없는 사용자가 상품을 생성하려 하면 실패한다
    When 권한 없는 사용자가 "테스트 상품" 상품을 생성한다
