Feature: 상품 관리
  ...
  Scenario: 관리자가 상품을 이름으로 검색한다
    Given 재고가 10개인 "장작" "SALE" 상품이 등록되어 있다
    And 재고가 5개인 "부탄가스" "SALE" 상품이 등록되어 있다
    When 관리자가 "장작" 키워드로 상품을 검색하면
    Then 검색 결과에 "장작" 상품이 포함되어야 한다
    And 검색 결과에 "부탄가스" 상품이 포함되지 않아야 한다
