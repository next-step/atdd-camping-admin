...
public class reservationStatusSteps {
    ...
    @When("^예약 상태를 체크인으로 변경한다 \\(PATCH \"/admin/reservations/\\{id\\}/status\"\\)$")
    ...
    @When("^예약 상태를 체크아웃으로 변경한다 \\(PATCH \"/admin/reservations/\\{id\\}/status\"\\)$")
    ...
    @When("^예약 상태를 확정으로 변경한다 \\(PATCH \"/admin/reservations/\\{id\\}/status\"\\)$")
    ...
    @When("^예약 상태를 대기로 변경한다 \\(PATCH \"/admin/reservations/\\{id\\}/status\"\\)$")
    ...
    @When("^현재와 동일한 상태로 변경을 시도한다 \\(PATCH \"/admin/reservations/\\{id\\}/status\"\\)$")
    ...
    @When("^존재하지 않는 상태 값으로 변경을 시도한다 \\(PATCH \"/admin/reservations/\\{id\\}/status\"\\)$")
    ...
    @When("^소문자로 된 상태 값으로 변경을 시도한다 \\(PATCH \"/admin/reservations/\\{id\\}/status\"\\)$")
    ...
    @When("^숫자 값으로 상태 변경을 시도한다 \\(PATCH \"/admin/reservations/\\{id\\}/status\"\\)$")
    ...
    @When("^특수문자가 포함된 상태 값으로 변경을 시도한다 \\(PATCH \"/admin/reservations/\\{id\\}/status\"\\)$")
    ...
    @When("^SQL 인젝션 문자열로 상태 변경을 시도한다 \\(PATCH \"/admin/reservations/\\{id\\}/status\"\\)$")
    ...
    @When("^빈 문자열로 상태 변경을 시도한다 \\(PATCH \"/admin/reservations/\\{id\\}/status\"\\)$")
    ...
    @When("^공백 문자열로 상태 변경을 시도한다 \\(PATCH \"/admin/reservations/\\{id\\}/status\"\\)$")
    ...
    @When("^status 필드 없이 상태 변경을 시도한다 \\(PATCH \"/admin/reservations/\\{id\\}/status\"\\)$")
    ...
    @When("^빈 요청 본문으로 상태 변경을 시도한다 \\(PATCH \"/admin/reservations/\\{id\\}/status\"\\)$")
    ...
    @When("^존재하지 않는 예약의 상태를 변경한다 \\(PATCH \"/admin/reservations/\\{id\\}/status\"\\)$")
    ...
    @Then("^상태 변경에 성공한다 \\((\\d+)\\)$")
    public void 상태변경에성공한다(int statusCode) {
        context.response.then().statusCode(statusCode);
    }

    @Then("^상태 변경이 거부된다 \\((\\d+)\\)$")
    public void 상태변경이거부된다(int statusCode) {
        context.response.then().statusCode(statusCode);
    }
